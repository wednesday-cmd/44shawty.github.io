...
1)?d.pos=m:d.pos=g}k.defineMode("perl",function(){function d(a,c,b,d,e){c.chain=null;c.style=null;c.tail=null;c.tokenize=function(a,c){for(var f=!1,g,h=0;g=a.next();){if(g===b[h]&&!f){void 0!==b[++h]?(c.chain=b[h],c.style=d,c.tail=e):e&&a.eatWhile(e);c.tokenize=n;break}f=!f&&"\\"==g}return d};return c.tokenize(a,c)}function k(a,c,b){c.tokenize=function(a,c){a.string==b&&(c.tokenize=n);a.skipToEnd();return"string"};return c.tokenize(a,c)}function n(a,c){var b;if(a.eatSpace())return null;if(c.chain)return d(a,
c,c.chain,c.style,c.tail);if(a.match(/^\-?[\d\.]/,!1)&&a.match(/^(\-?(\d*\.\d+(e[+-]?\d+)?|\d+\.\d*)|0x[\da-fA-F]+|0b[01]+|\d+(e[+-]?\d+)?)/))return"number";if(a.match(/^<<(?=\w)/))return a.eatWhile(/\w/),k(a,c,a.current().substr(2));if(a.sol()&&a.match(/^\=item(?!\w)/))return k(a,c,"=cut");var h=a.next();if('"'==h||"'"==h){if(q(a,3)=="<<"+h){var l=a.pos;a.eatWhile(/\w/);if((b=a.current().substr(1))&&a.eat(h))return k(a,c,b);a.pos=l}return d(a,c,[h],"string")}if("q"==h&&(b=g(a,-2),!b||!/\w/.test(b)))if(b=
