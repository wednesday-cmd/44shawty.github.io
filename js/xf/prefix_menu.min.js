...
(a=this.$menu.data("menu-trigger"))&&a.close();a=this.$title;a.length&&a.autofocus()}});XF.PrefixLoader=XF.Element.newHandler({options:{listenTo:"",href:""},init:function(){if(!this.$target.is("select"))console.error("Must trigger on select");else if(this.options.href){var a=this.options.listenTo?XF.findRelativeIf(this.options.listenTo,this.$target):b([]);if(a.length)a.on("change",b.proxy(this,"loadPrefixes"));else console.error("Cannot load prefixes dynamically as no element set to listen to for changes")}},
