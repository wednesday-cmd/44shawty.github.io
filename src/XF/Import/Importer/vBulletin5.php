...
class vBulletin5 extends vBulletin4
{
	...
	protected $forumRootId = 1; // TODO: pretty sure this is fixed, but it might be variable if the data is from an upgraded vB3/4..?
	...
	protected function getForums()
	{
		...
		/*
		 * This will query all 'Channels' that can contain threads.
		 * This also assumes that there is a node with id=1 which is the 'Forums' parent.
		 * We will select the data, and vB3-ise it for ease of use
		 */
		...
		return $this->sourceDb->fetchAllKeyed("
			SELECT
				...
				IF(node.displayorder > 0, 1, 0) AS options,
				node.title, node.description, node.parentid, node.displayorder,
				channel.category
			FROM {$this->prefix}node AS
				node
			INNER JOIN {$this->prefix}channel AS
				channel ON(channel.nodeid = node.nodeid)
			WHERE node.protected = 0 
			AND node.parentid <> 0
			AND node.parentid <> ?
			AND node.nodeid <> ?
			AND channel.category <> IF(node.parentid = 1, 0, -1)
		", 'forumid', [$specialNodeId, $specialNodeId]);

		/*
		 * Notes:
		 * This query also fetches pseudo-forums like 'Blogs', 'Groups' and 'Uncategorized Groups'
		 * These could potentially contain threads, so we'll grab them and treat them as forums for
		 * the sake of importing as much data as possible.
		 */
	}
	...
	{
		// vB5 doesn't appear to support link forums at all, so override the original setupNodeType()
	...
	{
		$moderators = parent::getModerators();

		foreach ($moderators AS $i => $moderator)
		{
			$moderators[$i]['forumid'] = $moderator['nodeid'];
			unset($moderators[$i]['nodeid']);
		}
	...
	{
		$forumIds = $this->sourceDb->quote(array_keys($this->typeMap('node')));

		return $this->sourceDb->fetchOne("
			SELECT MAX(nodeid) FROM {$this->prefix}node
		#	INNER JOIN {$this->prefix}text USING(nodeid)
			WHERE parentid IN({$forumIds})
			AND nodeid NOT IN({$forumIds})
			AND nodeid = starter
		") ?: 0;
	}
	...
	protected function getThreads($startAfter, $end, $limit)
	{
		...
		#	INNER JOIN {$this->prefix}text AS text ON(text.nodeid = node.nodeid)
			...
			LIMIT {$limit}
		", 'threadid', [$startAfter, $end, $redirectType]);
	}
	...
	protected function rewriteQuotes($text)
	{
		...
		if (stripos($text, '[quote=') !== false)
		{
			...
			else if (preg_match($re3, $text)) // vB5 style
			{
				return preg_replace_callback(
					$re3,
					function ($match) {
